...
async function fetchWithRetry() {
  return retry(() => fetch('https://api.example.com/data'), 3, 1000);
}
```
...
program.name('mycli').description('My awesome CLI tool').version(pkg.version);
...
async function createProject() {
  ...
  await fs.writeFile(`${answers.name}/package.json`, JSON.stringify(packageJson, null, 2));
...
describe('User API', () => {
  describe('POST /api/users', () => {
    ...
    it('should return 400 for invalid data', async () => {
      const response = await request(app).post('/api/users').send({ name: 'John' }).expect(400);
  ...
  describe('GET /api/users/:id', () => {
    ...
    it('should return 404 for non-existent user', async () => {
      const response = await request(app).get('/api/users/507f1f77bcf86cd799439011').expect(404);
    });
  });
});
```
